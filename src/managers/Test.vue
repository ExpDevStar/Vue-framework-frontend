  ...
  <div :class="[$style.wrapper]" v-if="ready && loggedin && testsReady">
    <div :class="[$style.start]" v-if="!started">
      <template v-if="olympName">
        ...
        <div :class="[$style.start__description]">
          После начала олимпиады у Вас будет
          <b>{{ duration | clock('m') }}</b> для
          написания программ и отправки их на проверку. Обратите вниание,
          что после окончания таймера, отправить задания будет невозможно.
          Правильно планируйте своё время!

          <ul :class="[$style.rules]">
            <li>После посадки в аудиторию, участник до сдачи выполненной работы имеет право не более одного раза выйти в туалет на срок до десяти минут в сопровождении одного из организаторов. В этом случае участник должен сдать организаторам телефон и паспорт.</li>
            <li>На рабочем месте участника при выполнении задания могут находиться только паспорт, черновики, авторучки с пастой черного или синего цвета и, при необходимости, чертежные инструменты, не имеющие записей в виде формул.</li>
            <li>Запрещается вставать с мест, пересаживаться, разговаривать с другими участниками и решать задачи других вариантов.</li>
            <li>Запрещается использовать источники информации, средства оперативной связи и вычислительной техники.</li>
            <li>За нарушение правил поведения на олимпиаде, в частности за пользование шпаргалками или запрещенной техникой, участник удаляется независимо от числа правильно решенных заданий без проверки работы. При удалении за пользование шпаргалкой, последняя обязательно прикладывается к работе (позже она может быть возвращена участнику).</li>
            <li>При завершении олимпиады участник расписывается об успешности отправки файлов.</li>
          </ul>
        </div>
      </template>
      <template v-else>
